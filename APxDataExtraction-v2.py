...
class APxAutomation:
    ...
    # Updated sanitize_sheet_name function with proper truncation
    @staticmethod
    ...
    # Improved abbreviate_name function to handle edge cases
    @staticmethod
    def abbreviate_name(name, max_length=10):
        ...
        abbreviation = ''.join([word[0].upper() for word in words if word])  # Ensure words are not empty
        if len(abbreviation) <= max_length:
            return abbreviation
        return abbreviation[:max_length]

    # Ensuring uniqueness of sheet names and avoid duplicates
    def unique_sheet_name(self, workbook, base_name):
        sanitized_name = self.sanitize_sheet_name(base_name)
        count = 2
        sheet_name = sanitized_name
        while sheet_name in workbook.sheetnames:
            sheet_name = f"{sanitized_name} ({count})"
            count += 1
        return sheet_name 
    ...
    # More robust append_or_export_to_excel function with checks
    def append_or_export_to_excel(self, checked_signal_paths, unit_descriptor=None, filename=None, args=None):
        file_name = filename if filename else "exported_data.xlsx"
        if not file_name.endswith('.xlsx'):  # Ensure proper extension
            file_name += '.xlsx'
        
        if os.path.exists(file_name):
            try:
                self.append_to_existing_excel(file_name, checked_signal_paths, unit_descriptor)
                logging.info(f"Appended data to {file_name}.")
            except Exception as e:
                logging.error(f"Error appending data to {file_name}: {e}")
        else:
            # Wrap in try-except to handle potential errors during export
            try:
                self.export_to_excel(checked_signal_paths, unit_descriptor, filename, args)
                logging.info(f"Exported data to {file_name}.")
            except Exception as e:
                logging.error(f"Error exporting data to {file_name}: {e}")
    ...
    def append_to_existing_excel(self, filename, checked_signal_paths, unit_descriptor):
        ...
        if hasattr(self, 'unitInput') and hasattr(self.unitInput, 'Text'):
            unit_no = self.unitInput.Text.strip()
        else:
            unit_no = "DefaultUnitNumber"  # Replace with your default or error handling
                ...
                for res_idx, result in enumerate(measurement["results"]):
                    ...
                    if 'xValues' in result['data'] and 'yValues' in result['data']:
                        ...
                        for channel_idx, y_set in enumerate(yValues):
                            ...
                            ws.cell(row=5, column=col, value=f"{cleaned_descriptor} Ch{channel_idx+1}")  # <-- Adjusted this line to fix naming convention
                            for row_idx, y_val in enumerate(y_set):
                                ws.cell(row=row_idx + 6, column=col, value=y_val)
                            col += 1
                    ...
                    if 'meterValues' in result['data']:
                        ...
                        meter_ws.cell(row=5, column=last_column, value=unit_descriptor)
                        for idx, val in enumerate(result['data']['meterValues'], start=6):
        ...
        try:
            ...
            logging.error(f"Failed to save the workbook '{filename}'. Error: {e}")
...
if __name__ == "__main__":
    try:
        ...
        if checked_data:
            apx_auto.append_or_export_to_excel(checked_data, args.description, args.filename)
        else:
